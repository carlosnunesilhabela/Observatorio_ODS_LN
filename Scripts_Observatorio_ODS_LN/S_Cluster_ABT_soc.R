...
ABT_soc <- readxl::read_xlsx("ABT_municipios_social_std.xlsx")

# remove(ABT_eco)

#Calcular a matriz de distancias euclidianas os municipios
dist_euclid <- dist(ABT_soc[5:19], method="euclidean")  #Calculo das distancias euclidianas
...
ABT_soc$cluster_soc <- as.factor(cutree(clust_res, k=5))  #corta em 5

#Tamanho dos Clusters
table(ABT_soc$cluster_soc)
...
writexl::write_xlsx(ABT_soc, "Cluster_soc.xlsx")
